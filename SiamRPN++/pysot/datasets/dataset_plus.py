...
def init_lmdbs():
    lmdbs = {}
    base_path = "/media/ubuntu/dataset/object_tracking/crop_datas"
    for name in cfg.DATASET.NAMES:
        key = name
        if name == "YOUTUBEBB":
            name = "YTB"
        path = "%s/%s/lmdb_%s" % (base_path, name, name.lower())
        lmdbs[key] = lmdb.open(path)
    return lmdbs
...
class SubDataset(object):
    ...
    def get_image_anno(self, video, track, frame):
        ...
        image_path = os.path.join(video,
                                  self.path_format.format(frame, track, 'x'))
        image_anno = self.labels[video][track][frame]
        img = self.get_image(image_path)
        return img, image_anno

    def get_image(self, key):
        with self.env.begin(write=False) as txn:
            buf = txn.get(key.encode("ascii"))
        imgbuf = pa.deserialize(buf)
